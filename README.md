...
- **邮件通知**：只在真正执行下单动作后通知（成功/失败/状态无法确认），邮件标题即「关键词 + 价格」；另有风控暂停、初始化完成等一次性告警；另支持**微信 clawbot 推送**（`clawbot.enabled` 开关，token 走环境变量 `REASONIX_BOT_CONTROL_TOKEN`）
